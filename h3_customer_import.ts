...
export function getInputData(context: EntryPoints.MapReduce.getInputDataContext) {
    ...
    log.debug("customer_import.getInputData => maxEsModDate", maxEsModDate);

    return wrapper.getCustomers(maxEsModDate, esConfig);
}

export function process(wrapper: any, esCustomer: any) {

    log.debug("process => esCustomer", esCustomer);

    const { store, filters, esConfig } = init();
    const { esId, esModDate, recType } = esCustomer;
    ...
    try {
        const nsCustomer = nsId ?
            record.load({ type: recType, id: nsId, isDynamic: true }) :
            record.create({ type: recType, isDynamic: true });

        for (const value of esConfig[EXTERNAL_STORES_CONFIG.KEYS.CUSTOMER_IMPORT_FUNCTION] as string[]) {
            const values = value.split(/\s+/);
            const functionName = values[0];
            const args = values.slice(1);
            const _function = wrapper[functionName] || functions[functionName];
            _function && _function.apply({ nsRecord: nsCustomer, esRecord: esCustomer, esConfig }, args);
        }

        esCustomer.nsId = nsId = String(nsCustomer.save({
        ...
        log.debug("Success", `${RECORDS_SYNC.VALUES.RECORD_TYPES.CUSTOMER} with id ${nsId}, ${RECORDS_SYNC.VALUES.STATUSES.IMPORTED}`);
...
export function map(context: EntryPoints.MapReduce.mapContext) {
    const wrapper = getWrapper();
    const esCustomer = wrapper.parseCustomer(context.value);
    process(wrapper, esCustomer);
    wrapper.shouldReduce?.(context, esCustomer);
}
